...
|Call|Inputs|Notes|
| --- | --- | --- |
|`TittaLSL::Sender()` (C++)<br>`TittaLSL.Sender()` (MATLAB)<br>`TittaLSLPy.Sender` (Python)|<ol><li>`address`: address of the eye tracker to be made available on the network. A list of connected eye trackers and their addresses can be using the static function `Titta.findAllEyeTrackers()` in the Titta library.</li></ol>||
...
The following static calls are available on a `TittaLSL.Receiver` instance:
|Call|Inputs|Outputs|Description|
| --- | --- | --- | --- |
...
The following method calls are available on a `TittaLSL.Receiver` instance. Note that samples provided by the `consume*` and `peek*` functions is almost identical to that provided by their namesakes in `Titta` for a local eye tracker. The only difference is that the samples provided by TittaLSL have two extra fields, `remoteSystemTimeStamp` and `localSystemTimeStamp`. `remoteSystemTimeStamp` is the timestamp as provided by the Tobii SDK on the system where the eye tracker is connected. `localSystemTimeStamp` is this same timestamp expressed in time of the receiving machine, corrected by using the offset provided by Lab Streaming Layer's `time_correction` function. See [the Tobii SDK documentation](https://developer.tobiipro.com/commonconcepts.html) for a description of the other fields.
