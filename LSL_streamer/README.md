...
|Call|Inputs|Outputs|Description|
| --- | --- | --- | --- |
|`GetStreams()`|<ol><li>`stream`: (optional) string, possible values: `gaze`, `externalSignal`, `timeSync` and `positioning`. If provided, only streams of this type are discovered on the network.</li></ol>|<ol><li>`streamInfoList`: list of objects containing info about discovered streams.</li></ol>|Discover what TittaLSL streams are available on the network.|
